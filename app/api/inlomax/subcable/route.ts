...
export async function POST(request: NextRequest) {
  try {
    ...
    const { service, smartcard_number, plan_id, amount } = body;

    if (!service || !smartcard_number || !plan_id) {
      return NextResponse.json(
        { error: 'Missing required fields: service, smartcard_number, plan_id' },
        { status: 400 }
      );
    }
    ...
    try {
      // Call Inlomax API
      const result = await inlomax.subCable({
        service,
        smartcard_number,
        plan_id,
        reference,
      ...
      return NextResponse.json(
        { error: 'Cable subscription failed', details: apiError instanceof Error ? apiError.message : 'Unknown error' },
        { status: 500 }
      );
    }
  } catch (error) {
